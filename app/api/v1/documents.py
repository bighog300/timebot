...
def reprocess_document(document_id: UUID, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    enforce_rate_limit(db, user_id=current_user.id, metric="processing_requests_rate", max_calls=configured_rate_limit("processing_requests_rate"))
...
def regenerate_document_intelligence(document_id: UUID, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    enforce_rate_limit(db, user_id=current_user.id, metric="processing_requests_rate", max_calls=configured_rate_limit("processing_requests_rate"))
    enforce_daily_cap(db, user_id=current_user.id, metric="processing_jobs_daily", cap=hard_daily_caps()["processing_jobs_daily"])
    enforce_feature(db, current_user.id, "insights_enabled")
    enforce_limit(db, current_user.id, "processing_jobs_per_month", quantity=1)
    try:
        result = document_intelligence_service.regenerate(db, document)
        record_usage(db, user_id=current_user.id, metric="processing_requests_rate", quantity=1)
        record_usage(db, user_id=current_user.id, metric="processing_jobs_daily", quantity=1, metadata={"source": "api_intelligence_regenerate"})
        db.commit()
        return result
