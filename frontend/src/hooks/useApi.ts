...
export function useChatPromptTemplates(assistantId?: string) { const authReady = useAuthReady(); return useQuery({ queryKey: [...keys.chatPromptTemplates, assistantId || 'all'], queryFn: () => api.listChatPromptTemplates(assistantId), enabled: authReady }); }
