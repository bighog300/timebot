...
export const api = {
  ...
  inviteWorkspaceMember: async (workspaceId: string, payload: { email: string; role: string }): Promise<{ invite: WorkspaceInvite; dev_invite_link?: string | null }> => (await http.post(`/workspaces/${workspaceId}/invites`, payload)).data,
  acceptWorkspaceInvite: async (token: string): Promise<{ accepted: boolean }> => (await http.post(`/workspaces/invites/${token}/accept`)).data,
  resendWorkspaceInvite: async (workspaceId: string, inviteId: string): Promise<WorkspaceInvite> => (await http.post(`/workspaces/${workspaceId}/invites/${inviteId}/resend`)).data,
  cancelWorkspaceInvite: async (workspaceId: string, inviteId: string): Promise<void> => { await http.delete(`/workspaces/${workspaceId}/invites/${inviteId}`); },
  ...
  createChatSession: async (payload: CreateChatSessionRequest = {}): Promise<ChatSession> => (await http.post('/chat/sessions', payload)).data,
  listAssistants: async (): Promise<AssistantProfile[]> => (await http.get('/chat/assistants')).data,
  ...
  getBillingStatus: async (): Promise<{ provider: string; stripe_enabled: boolean; enabled: boolean }> => (await http.get('/billing/status')).data,
  ...
  createCustomerPortalSession: async (): Promise<{ portal_url: string }> => (await http.post('/billing/portal')).data,
  ...
  getAuthConfig: async (): Promise<AuthConfig> => (await http.get('/auth/config')).data,
  ...
  previewEmailTemplate: async (payload: EmailTemplatePreviewRequest): Promise<EmailTemplatePreviewResponse> => (await http.post('/admin/email/templates/preview', payload)).data,
  duplicateEmailTemplate: async (templateId: string): Promise<EmailTemplate> => (await http.post(`/admin/email/templates/${templateId}/duplicate`)).data,
  testSendEmailTemplate: async (payload: EmailTemplateTestSendRequest): Promise<EmailTestSendResult> => (await http.post('/admin/email/templates/test-send', payload)).data,
