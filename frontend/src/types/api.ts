...
export type SystemIntelligenceSubmissionStatus = 'pending' | 'approved' | 'rejected' | 'withdrawn';

export interface SystemIntelligenceSubmission {
  id: string;
  status: SystemIntelligenceSubmissionStatus;
  source_document_id: string;
  source_document_title?: string | null;
  ...
  reviewed_at?: string | null;
  resulting_system_document_id?: string | null;
  created_at?: string;
}
...
export interface ChatPromptTemplateSummary { id: string; name: string; type: string; required_plan?: string | null; assistant_id?: string | null; is_default?: boolean; locked?: boolean; }
...
export interface DivorceTimelineEvent { id:string; workspace_id:string; event_date?:string|null; date_precision:'exact'|'approximate'|'inferred'|'unknown'; title:string; description?:string|null; category:string; source_document_id?:string|null; source_quote?:string|null; source_snippet?:string|null; confidence?:number|null; review_status:'suggested'|'accepted'|'rejected'|'edited'; include_in_report:boolean; metadata_json?:Record<string, unknown>|null }

export interface DivorceCommunication { id:string; workspace_id:string; source_document_id?:string|null; source_email_id?:string|null; source_message_id?:string|null; sender:string; recipient?:string|null; subject?:string|null; sent_at?:string|null; communication_type:'email'|'message'|'note'|'document'; category:'lawyer'|'spouse_or_other_party'|'court'|'financial'|'children'|'school'|'mediator'|'unknown'; tone:'neutral'|'hostile'|'cooperative'|'urgent'|'threatening'|'unclear'; extracted_issues_json?:Record<string, unknown>|null; extracted_deadlines_json?:unknown[]|null; extracted_offers_json?:unknown[]|null; extracted_allegations_json?:unknown[]|null; confidence?:number|null; review_status:'suggested'|'accepted'|'rejected'|'edited'; metadata_json?:Record<string, unknown>|null }

export interface DivorceReport { id:string; workspace_id:string; report_type:string; title:string; status:string; content_markdown:string; source_task_ids_json:string[]; source_timeline_item_ids_json:string[]; source_document_ids_json:string[]; error_message_sanitized?:string|null; metadata_json?:Record<string, unknown>|null; created_at:string }
export interface DivorceDashboard { documents_uploaded:number; emails_imported:number; open_tasks:number; suggested_task_count:number; open_task_count:number; urgent_task_count:number; upcoming_due_dates: DivorceTask[]; recently_generated_tasks: DivorceTask[]; upcoming_deadlines: unknown[]; key_timeline_events: unknown[]; latest_reports: DivorceReport[]; report_count:number; failed_report_count:number; missing_information_checklist: string[]; suggested_timeline_count:number; accepted_timeline_count:number; upcoming_timeline_events: DivorceTimelineEvent[]; recent_timeline_events: DivorceTimelineEvent[]; high_confidence_event_count:number }

export interface DivorceAdvisorMapping { key:string; label:string; assistant_id:string; assistant_name:string; prompt_template_id:string; prompt_template_name:string; locked:boolean; required_plan:string; chat_title:string }
