...
const mocks = vi.hoisted(()=>({create:vi.fn().mockResolvedValue({id:'s1'})}));
vi.mock('react-router-dom', async (orig)=>({...(await orig() as object), useNavigate:()=>nav}));
vi.mock('@/services/api', ()=>({api:{
  getDivorceDashboard: vi.fn().mockResolvedValue({documents_uploaded:1,suggested_task_count:1,open_task_count:0,suggested_timeline_count:0,accepted_timeline_count:0,report_count:0}),
