...
export function DivorceTasksPage(){
  ...
  return <div className='space-y-4'><h1 className='text-xl font-semibold'>Divorce Tasks</h1>
    ...
    {suggested.map(t=><div key={t.id} className='rounded border p-3 text-sm'><div>{t.content}</div><div>{t.category} · {t.priority} · {t.due_date||'No due date'}</div><div>{t.source_snippet||t.source_quote}</div><button onClick={()=>accept.mutate(t.id)}>Accept</button> <button onClick={()=>reject.mutate(t.id)}>Reject</button></div>)}
    <h2 className='font-medium'>Open/Active tasks ({rest.length})</h2>
    {rest.map(t=><div key={t.id} className='rounded border p-3 text-sm'><div>{t.content}</div><div>{t.status} · {t.category} · {t.priority} · {t.due_date||'No due date'}</div><button onClick={()=>mark.mutate({id:t.id,status:'done'})}>Mark done</button> <button onClick={()=>mark.mutate({id:t.id,status:'dismissed'})}>Dismiss</button></div>)}
  </div>
}
