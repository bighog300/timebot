...
export function LoginPage() {
  ...
  useEffect(() => {
    api
      .getAuthConfig()
      .then((cfg) => {
        setGoogleEnabled(Boolean(cfg.google_login_enabled));
        setLocalEnabled(Boolean(cfg.local_login_enabled));
      })
      .catch(() => {
  ...
  return (
    <div className="mx-auto mt-16 max-w-md rounded border border-slate-800 bg-slate-900 p-6">
      ...
      {localEnabled ? (
        ...
        <p className="rounded border border-amber-700 bg-amber-950/40 p-3 text-sm text-amber-200">Local email/password login is disabled for this deployment.</p>
      )}
      {googleEnabled && <button className='mt-3 w-full rounded bg-rose-600 p-2' onClick={() => setError('Google sign-in is enabled, but this client does not yet implement OAuth redirect. Use local login if available.')}>Continue with Google</button>}
