...
export function ChatPage() {
  ...
  const onCreate = async () => {
    ...
    if (chosenTemplate?.locked) {
      setMonetizationError(`Upgrade required to use template ${chosenTemplate.name}.`);
      setUpgradeModal({ feature: 'prompt templates', requiredPlan: chosenTemplate.required_plan || 'pro', message: `Upgrade to ${String(chosenTemplate.required_plan || 'pro').toUpperCase()} to use ${chosenTemplate.name}.` });
      return;
    }
    if (chosenTemplate && assistantId && chosenTemplate.assistant_id && chosenTemplate.assistant_id !== assistantId) {
      setMonetizationError('Selected template is incompatible with the selected assistant.');
      return;
    }
