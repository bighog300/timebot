...
export function AdminPromptTemplatesPage() {
  ...
  const [testForm, setTestForm] = useState<{ type: PromptTemplateType; content: string; sample_context: string; provider: 'openai' | 'gemini'; model: string; temperature: number; max_tokens: number; top_p: number; fallback_enabled: boolean; fallback_order: 'provider_then_model'|'model_then_provider'; max_fallback_attempts: number; retry_on_provider_errors: boolean; retry_on_rate_limit: boolean; retry_on_validation_error: boolean; fallback_provider: 'openai' | 'gemini' | null; fallback_model: string | null }>({ type: 'chat', content: '', sample_context: '', provider: 'openai', model: 'gpt-4o-mini', temperature: 0.2, max_tokens: 800, top_p: 1, fallback_enabled: false, fallback_order: 'provider_then_model', max_fallback_attempts: 1, retry_on_provider_errors: true, retry_on_rate_limit: true, retry_on_validation_error: false, fallback_provider: 'openai', fallback_model: 'gpt-4.1-mini' });
  ...
  return <div className='space-y-4'>
    ...
    <div className='space-y-2 rounded border border-slate-800 p-3'>
      ...
      <select value={testForm.type} onChange={(e) => setTestForm((v) => ({ ...v, type: e.target.value as PromptTemplateType }))} className='rounded border border-slate-700 bg-slate-900 p-2 text-sm'>
        {promptTypes.map((type) => <option key={type} value={type}>{type}</option>)}
      </select>
      <textarea placeholder='Prompt content for preview' value={testForm.content} onChange={(e) => setTestForm((v) => ({ ...v, content: e.target.value }))} className='h-32 w-full min-w-0 rounded border border-slate-700 bg-slate-900 p-2 text-sm' />
      ...
      <button className='w-full rounded bg-indigo-700 px-3 py-2 text-sm sm:w-auto' onClick={async () => {
        ...
        if (!testForm.content.trim() || !testForm.sample_context.trim()) {
      ...
      }} disabled={testPrompt.isPending || !testForm.content.trim() || !testForm.sample_context.trim()}>Run preview</button>
    ...
    {selectedPrompt && <div className='space-y-2 rounded border border-slate-800 p-3'>
      ...
      <div className='text-xs text-slate-400'>{selectedPrompt.type} · v{selectedPrompt.version}</div>
      ...
      <button className='w-full rounded bg-indigo-700 px-3 py-2 text-sm sm:w-auto' onClick={async () => { try { const payload = { ...editForm }; delete (payload as { type?: PromptTemplateType }).type; await updatePrompt.mutateAsync({ promptId: selectedPrompt.id, payload }); pushToast('Prompt template saved'); } catch (e) { pushToast(getErrorDetail(e), 'error'); } }}>Save changes</button>
    </div>}
  </div>;
}
